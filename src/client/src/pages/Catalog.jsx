...
function Catalog() {
  ...
  // Load user equipment from localStorage and API, fetch filter options on mount
  useEffect(() => {
  ...
  return (
    <div className="p-4 space-y-4 pb-24">
      ...
      {/* Quick Filters Row - Equipment & Favorites */}
      {(userEquipment.length > 0 || favoriteIds.size > 0) && (
        <div className="grid grid-cols-2 gap-2">
          {/* My Equipment Toggle */}
          {userEquipment.length > 0 && (
            <div className="flex items-center justify-between bg-dark-card rounded-xl p-2.5 sm:p-3">
              <div className="flex items-center gap-2">
                <svg className="w-4 h-4 sm:w-5 sm:h-5 text-accent flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
                </svg>
                <p className="text-white text-xs sm:text-sm font-medium">My Equipment</p>
                <div className="relative group">
                  ...
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-dark-elevated text-xs text-gray-300 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                    Hide exercises requiring equipment I don't have
                  </div>
                </div>
              </div>
              <button
                onClick={() => setMyEquipmentOnly(!myEquipmentOnly)}
                className={`w-10 h-6 sm:w-12 sm:h-7 rounded-full transition-colors relative flex-shrink-0 ${
                  myEquipmentOnly ? 'bg-accent' : 'bg-dark-elevated'
                }`}
              >
                <div className={`absolute top-1 w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-white transition-transform ${
                  myEquipmentOnly ? 'translate-x-5 sm:translate-x-6' : 'translate-x-1'
                }`} />
              </button>
            </div>
          )}
          ...
          {favoriteIds.size > 0 && (
            <div className={`flex items-center justify-between bg-dark-card rounded-xl p-2.5 sm:p-3 ${userEquipment.length === 0 ? 'col-span-2 sm:col-span-1' : ''}`}>
              <div className="flex items-center gap-2">
                <svg className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-400 flex-shrink-0" fill="currentColor" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                </svg>
                <p className="text-white text-xs sm:text-sm font-medium">Favorites <span className="text-gray-500">({favoriteIds.size})</span></p>
                <div className="relative group">
                  <svg className="w-3.5 h-3.5 text-gray-500 cursor-help" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-dark-elevated text-xs text-gray-300 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                    Show only exercises I've starred
                  </div>
                </div>
              </div>
              <button
              ...
              >
                <div className={`absolute top-1 w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-white transition-transform ${
                  favoritesOnly ? 'translate-x-5 sm:translate-x-6' : 'translate-x-1'
                }`} />
              </button>
            </div>
          )}
